...
const VendorDashboard = () => {
  ...
  const saveVerifiedDate = async (item) => {
    const token = localStorage.getItem("vendor_token");
    setActionId(item.id);
    try {
      const response = await fetch(`${API}/vendor/cases/verified-date`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ vendor_id: vendor.id, _token: token, client_application_id: item.id, verified_date: verifiedDates[item.id] || item.vendor_verified_date || "" }),
      });
      const result = await response.json();
  ...
  const renderCell = (item, key, type) => {
    ...
    if (type === "verifiedDate") {
      return <div className="flex gap-2 items-center">
        <input type="date" className="border rounded px-2 py-1" value={verifiedDates[item.id] ?? (item.vendor_verified_date ? String(item.vendor_verified_date).slice(0, 10) : "")} onChange={(event) => setVerifiedDates((prev) => ({ ...prev, [item.id]: event.target.value }))} disabled={Number(item.vendor_case_enabled) === 0} />
        <button onClick={() => saveVerifiedDate(item)} disabled={actionId === item.id || Number(item.vendor_case_enabled) === 0} className="bg-green-600 text-white px-3 py-1 rounded disabled:opacity-50">Save</button>
      </div>;
    }
