  ...
  const fetchActiveAccounts = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://devscreeningnode.onrender.com/trashed/customer/list?admin_id=${admin_id}&_token=${storedToken}`,
  ...
  const handleDelete = useCallback((id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://devscreeningnode.onrender.com/trashed/customer/delete?id=${id}&admin_id=${admin_id}&_token=${storedToken}`, requestOptions)
  ...
  const handleRestore = useCallback((id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        // Perform the DELETE request to delete the customer
        fetch(`https://devscreeningnode.onrender.com/trashed/customer/restore/?id=${id}&admin_id=${admin_id}&_token=${storedToken}`, requestOptions)
          .then((response) => response.text())
          .then((result) => {
            const newToken = result.token || result._token || '';
            if (newToken) {
