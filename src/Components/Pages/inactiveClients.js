   ...
   const tableScrollRef = useRef(null);
    ...
    const fetchInactiveClients = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(
                `http://localhost:5000/customer/inactive-list?admin_id=${admin_id}&_token=${storedToken}`,
                {
                    method: "GET",
                    redirect: "follow",
                }
    ...
    const handleUnblock = async (id) => {
        ...
        if (confirmation.isConfirmed) {
            ...
            try {
                const response = await fetch(
                    `http://localhost:5000/customer/active?customer_id=${id}&admin_id=${admin_id}&_token=${storedToken}`,
                    {
                        method: "GET",
                        redirect: "follow",
                    }
