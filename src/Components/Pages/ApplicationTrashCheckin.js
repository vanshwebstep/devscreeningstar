...
const ApplicationTrashCheckin = () => {
    ...
    const fetchData = useCallback((filterStatus = null) => {
        ...
        const baseUrl = `https://devscreeningnode.onrender.com/trashed/client-master-tracker/applications-by-branch?branch_id=${branchId}&admin_id=${adminId}&_token=${token}`;
    ...
    const handleApplicationDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const url = `https://devscreeningnode.onrender.com/trashed/client-master-tracker/application-delete?application_id=${id}&admin_id=${adminId}&_token=${token}`;
                fetch(url, requestOptions)
                    .then((response) => {
                        if (!response.ok) {
                            throw new Error(`HTTP error! status: ${response.status}`);
    ...
    const handleApplicationRestore = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const url = `https://devscreeningnode.onrender.com/trashed/client-master-tracker/application-restore?application_id=${id}&admin_id=${adminId}&_token=${token}`;
                fetch(url, requestOptions)
                    .then((response) => {
                        if (!response.ok) {
                            throw new Error(`HTTP error! status: ${response.status}`);
