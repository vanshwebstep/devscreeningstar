  ...
  const fetchData = async (page = 1, search = '') => {
    ...
    try {
      const response = await fetch(
        `https://devscreeningnode.onrender.com/ticket/list?admin_id=${admin_id}&_token=${storedToken}&page=${page}&search=${search}`,
        requestOptions
      );
  ...
  const handleDelete = async (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setDeletingId(id);
        fetch(
          `https://devscreeningnode.onrender.com/ticket/delete?ticket_number=${id}&admin_id=${admin_id}&_token=${storedToken}`,
          requestOptions
        )
