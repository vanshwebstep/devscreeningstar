  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/tat-delay/list?admin_id=${admin_id}&_token=${storedToken}`,
        {
          method: 'GET',
      ...
      if (swalResult.isConfirmed) {
        ...
        try {
          const response = await axios.delete(
            `http://localhost:5000/tat-delay/delete`,
            {
              params: {
  ...
  const handleApplicationDelete = (customerid, applicationId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const url = `http://localhost:5000/tat-delay/delete-application?customer_id=${customerid}&admin_id=${adminId}&_token=${token}&application_id=${applicationId}`;
