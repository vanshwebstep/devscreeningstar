...
const ViewAdminTicket = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch(
                `http://localhost:5000/ticket/view?ticket_number=${ticketNumber}&sub_user_id=&admin_id=${admin_id}&_token=${storedToken}`,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/ticket/chat', {
                method: 'POST',
    ...
    const handleModalSubmit = () => {
        ...
        fetch("http://localhost:5000/ticket/update", requestOptions)
