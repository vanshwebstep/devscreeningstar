  ...
  const fetchRolesWithoutLoader = useCallback(async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/admin/permission/list?admin_id=${adminId}&_token=${token}`,
 ...
 const fecthPreselectServices=()=>{
  ...
  const handleUpdate = async (selectedServices) => {
    try {
      ...
      const response = await fetch("http://localhost:5000/admin/permission/update", requestOptions);
