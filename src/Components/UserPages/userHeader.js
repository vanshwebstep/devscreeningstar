...
const UserHeader = () => {
  ...
  const handleSignout = async () => {
    try {
      ...
      let url = `https://devscreeningnode.onrender.com/branch/logout?branch_id=${branchId}&_token=${branchtoken}`;
  ...
  const toggleNotificationPopup = useCallback(async () => {
    ...
    if (!isNotificationOpen) {
      ...
      try {
        ...
        const response = await fetch(`https://devscreeningnode.onrender.com/branch/notification?YnJhbmNoX2lk=${encodedBranchId}`);
