...
const CLientTrashed = () => {
    ...
    const fetchCustomerInfo = useCallback(async () => {
        ...
        const url = `http://localhost:5000/branch/trashed/client-application/listings?customer_id=${customer_id}&branch_id=${branchInfo.branch_id}&_token=${branch_token}`;
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (branchData?.type === "sub_user") {
                    ...
                    url = `http://localhost:5000/branch/trashed/client-application/delete?id=${id}&branch_id=${branch_id}&_token=${_token}&sub_user_id=${sub_user_id}`;
                } else {
                    url = `http://localhost:5000/branch/trashed/client-application/delete?id=${id}&branch_id=${branch_id}&_token=${_token}`;
                }
    ...
    const handleRestore = async (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (branchData?.type === "sub_user") {
                    const sub_user_id = branchData?.id ?? null;
                    url = `http://localhost:5000/branch/trashed/client-application/restore?id=${id}&branch_id=${branch_id}&_token=${_token}&sub_user_id=${sub_user_id}`;
                } else {
                    url = `http://localhost:5000/branch/trashed/client-application/restore?id=${id}&branch_id=${branch_id}&_token=${_token}`;
                }
