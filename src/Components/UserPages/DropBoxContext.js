...
const DataCheckin = () => {
    ...
    const fetchData = useCallback(() => {
        ...
        fetch(`https://devscreeningnode.onrender.com/data-management/applications-by-branch?branch_id=${branchId}&admin_id=${adminId}&_token=${token}`, requestOptions)
    ...
    const fetchServicesData = async (applicationId, servicesList, reportDownload = '0') => {
        ...
        try {
            ...
            const url = `https://devscreeningnode.onrender.com/client-master-tracker/services-annexure-data?service_ids=${encodeURIComponent(servicesList)}&report_download=${reportDownload}&application_id=${encodeURIComponent(applicationId)}&admin_id=${encodeURIComponent(adminId)}&_token=${encodeURIComponent(token)}`;
