...
const DigitalAddressVerification = () => {
    ...
    const isApplicationExists = useCallback(() => {
        ...
        if (
            ...
            fetch(
                `https://devscreeningnode.onrender.com/branch/candidate-application/digital-address-verification/is-application-exist?app_id=${decodedValues.app_id}&branch_id=${decodedValues.branch_id}&customer_id=${decodedValues.customer_id}`
            )
                .then(res => res.json())
                .then(result => {
                    if (!result.status) {
    ...
    const uploadCustomerLogo = async (
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            try {
                ...
                await axios.post(
                    "https://devscreeningnode.onrender.com/branch/candidate-application/digital-address-verification/upload",
    ...
    const handleSubmitForm = async (e) => {
        ...
        try {
            const response = await fetch(
                "https://devscreeningnode.onrender.com/branch/candidate-application/digital-address-verification/submit",
                {
                    method: "PUT",
