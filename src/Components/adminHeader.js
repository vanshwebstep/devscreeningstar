...
const AdminHeader = () => {
  ...
  const handleSignout = async () => {
    try {
      ...
      const url = `https://devscreeningnode.onrender.com/admin/logout?admin_id=${adminId}&_token=${storedToken}`;
  ...
  const toggleNotificationPopup = useCallback(async () => {
    ...
    if (!isNotificationOpen) {
      ...
      try {
        const response = await fetch(`https://devscreeningnode.onrender.com/notification?YWRtaW5faWQ=${encodedAdminId}`);
        if (!response.ok) {
  ...
  const toggleNotificationPopup2 = useCallback(async () => {
    ...
    try {
      const response = await fetch(`https://devscreeningnode.onrender.com/notification/view?YWRtaW5faWQ=${encodedAdminId}`);
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
      const result = await response.json();
  ...
  const handleCheckin = async () => {
    try {
      ...
      const response = await fetch("https://devscreeningnode.onrender.com/admin/check-in", requestOptions);
  ...
  const handleCheckout = async () => {
    try {
      ...
      const response = await fetch("https://devscreeningnode.onrender.com/admin/check-out", requestOptions);
      const result = await response.text();
      console.log(result);
